...
function wrapxml(xml)
{
  ...
  //temptopass = "<body xmlns:xml=\"http://www.w3.org/XML/1998/namespace\"><div xml:lang=\"en\" type=\"translation\" xml:space=\"preserve\">" + xml + "</div></body>";
  return temptopass;
}
  ...
  if (editpass == "yes")
    {
     ...
     // startxml = '<gap reason="' + type + '"' + ' extent="unknown" unit="character" 
      convertXML()
    }
} /*########################     end insertGapLost     ########################*/
...
function tryitConversion()
{
  ...
  if (tryit_type == "xml2non")
  ...
  else
    {
      ...
      new Ajax.Request(window.opener.convtranslation_leiden_to_xml, 
        {
          method: 'get',
