class DDBCurrentIdentifiersController < IdentifiersController
  # layout 'site'
  before_action :authorize
  before_action :ownership_guard, only: %i[update updatexml]

  # - GET /publications/1/ddb_current_identifiers/1/edit
    ...
    begin
      ...
      if fragment_exist?(action: 'edit', part: "leiden_plus_#{@identifier.id}")
        ...
        if Sosol::Application.config.respond_to?(:xsugar_standalone_enabled) && Sosol::Application.config.xsugar_standalone_enabled
          original_xml = DDBCurrentIdentifier.preprocess(@identifier.xml_content)
          ...
          abs = DDBCurrentIdentifier.get_div_edition(original_xml).join
  ...
  # PUT /publications/1/ddb_current_identifiers/1/rename
  ...
  # - PUT /publications/1/ddb_current_identifiers/1/update
  ...
  def update
    ...
    if params[:commit] == 'Save With Broken Leiden+' # Save With Broken Leiden+ button is clicked
      ...
      @is_editor_view = true
      render template: 'ddb_current_identifiers/edit'
      ...
      @identifier_html_preview =
        Epidocinator.apply_xsl_transform(
          Epidocinator.stream_from_string(
            DDBCurrentIdentifier.preprocess(@identifier.xml_content)
          ),
          {
            'xsl' => 'commentary'
          }
  ...
  # - GET /publications/1/ddb_current_identifiers/1/preview
  ...
  def find_identifier
    @identifier = DDBCurrentIdentifier.find(params[:id].to_s)
  end
