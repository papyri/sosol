...
class DclpTextIdentifiersController < IdentifiersController
  #layout 'site'
  ...
  def editmeta
    ...
    @dclpEpiDoc = DclpTextIdentifierHelper::DclpMeta::DclpEpiDoc.new(@identifier.xml_content)
    
    render :template => 'dclp_identifiers/editmeta'
  end
  ...
  # - GET /publications/1/dclp_identifiers/1/edit
  # - Edit DCLP Text via Leiden+
  def edit_likeDDBDP
    find_identifier
    ...
    begin
      ...
      if fragment_exist?(:action => 'edit', :part => "leiden_plus_#{@identifier.id}")
        ...
        if((!Sosol::Application.config.xsugar_standalone_enabled.nil?) && Sosol::Application.config.xsugar_standalone_enabled)
          original_xml = DCLPTextIdentifier.preprocess(@identifier.xml_content)

          # strip xml:id from lb's
          original_xml = JRubyXML.apply_xsl_transform(
            JRubyXML.stream_from_string(original_xml),
            JRubyXML.stream_from_file(File.join(Rails.root,
              %w{data xslt dclp strip_lb_ids.xsl})))

          # get div type=edition from XML in string format for conversion
          abs = DCLPTextIdentifier.get_div_edition(original_xml).join('')
  ...
  # PUT /publications/1/dclp_identifiers/1/rename
  # Executes the actual rename of the 'SoSOL' temporary identifer to the correct final name - ex. BGU, O.Ber, etc
  # Overrides base Identifier rename controller to special-case DCLP collections
  def rename
    find_identifier
  ...
  # - PUT /publications/1/dclp_identifiers/1/update
  # - Update DCLP Text via Leiden+
  def update
    find_identifier
    @bad_leiden = false
    ...
    if params[:commit] == "Save With Broken Leiden+" #Save With Broken Leiden+ button is clicked
      @identifier.save_broken_leiden_plus_to_xml(params[:dclp_identifier_leiden_plus].to_s, params[:comment].to_s)
      @bad_leiden = true
      flash.now[:notice] = "File updated with broken Leiden+ - XML and Preview will be incorrect until fixed"
      expire_leiden_cache
      expire_publication_cache
        @identifier[:leiden_plus] = params[:dclp_identifier_leiden_plus].to_s
        @is_editor_view = true
        render :template => 'dclp_identifiers/edit'
    else #Save button is clicked
      begin
        commit_sha = @identifier.set_leiden_plus(params[:dclp_identifier_leiden_plus].to_s,
                                    params[:comment].to_s)
        ...
        @identifier[:leiden_plus] = params[:dclp_identifier_leiden_plus]
        @is_editor_view = true
        render :template => 'dclp_identifiers/edit'
      end #begin
    end #when
  ...
  # Pull DCLP Text XML file from repository and creates a page set via XSLT up to add, modify, or delete
  ...
  def commentary
    ...
    JRubyXML.apply_xsl_transform(
      JRubyXML.stream_from_string(
        DCLPTextIdentifier.preprocess(@identifier.xml_content)),
      JRubyXML.stream_from_file(File.join(Rails.root,
        %w{data xslt dclp commentary.xsl})),
        {})
      
    @is_editor_view = true
  end
  
  # Updates a DCLP Text with Line by Line commentary
  # - *Params*  :
  ...
  #   - +content+ -> contains the commentary 'Leiden' to convert to XML that will be added to the DCLP Text XML
  ...
  # Updates a DCLP Text with Front Matter commentary
  # - *Params*  :
  #   - +content+ -> contains the commentary 'Leiden' to convert to XML that will be added to the DCLP Text XML
  # - *Returns* :
  #   - to commentary view
  ...
  # Deletes Front Matter commentary from a DCLP Text
  # - *Params*  :
  #   - +none+
  # - *Returns* :
  #   - to commentary view
  ...
  # Deletes Line by Line commentary from a DCLP Text
  # - *Params*  :
  #   - +line_id+ -> id attribute value of the 'li' tag this commentary is associated with
  #   - +reference+ -> defaults to numeric portion of 'line_id' - but user changable to reference multiple lines, etc. 
  #   - +content+ -> contains the commentary 'Leiden' to convert to XML that will be added to the DCLP Text XML
  ...
  # - GET /publications/1/dclp_identifiers/1/preview
  # - Provides preview of what the DCLP Text XML from the repository will look like with PN Stylesheets applied
  def preview
    find_identifier
  ...
  protected
    ...
    # - *Params*  :
    #   - +publication_id+ -> id from publication table of the publication containing this DCLP Text
