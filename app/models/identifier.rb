...
class Identifier < ApplicationRecord
  ...
  def commit_id_to_hash(commit_id)
    commit = {}
    commit[:id] = commit_id
    commit_data = Repository.run_command("#{self.repository.git_command_prefix} log -n 1 --pretty=format:\"%s%n%an%n%cn%n%at%n%ct\" #{commit_id}").split("\n")
  ...
  def self.next_temporary_identifier
    year = Time.now.year
    latest = Identifier.where("name like ?", "papyri.info/#{self::IDENTIFIER_NAMESPACE}/#{self::TEMPORARY_COLLECTION};#{year};%").order("name DESC").limit(1).first
    if latest.nil?
  ...
  def add_change_desc(text = "", user_info = self.publication.creator, input_content = nil, timestamp = Time.now.xmlschema)
    doc = JRubyXML.apply_xsl_transform(
      ...
      :who => url_helpers.url_for(:host => Sosol::Application.config.site_user_namespace, :controller => 'user', :action => 'show', :user_name => user_info.name, :only_path => false),
      :comment => text,
      :when => timestamp
    )
