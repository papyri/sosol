  ...
  def send_status_emails(when_to_send, publication)
    ...
    self.emailers.each do |mailer|
  	...
  	if mailer.send_to_owner
        ...
        begin
          EmailerMailer.identifier_email(when_to_send,email_identifiers,board_publication,addresses,mailer.include_document,mailer.include_comments,mailer.message,nil).deliver
        rescue Exception => e
