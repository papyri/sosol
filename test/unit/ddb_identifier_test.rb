...
if Sosol::Application.config.site_identifiers.split(',').include?('DDBIdentifier')
  class DDBIdentifierTest < ActiveSupport::TestCase
    ...
    context "identifier renaming" do
      ...
      context "with a valid target" do
        ...
        should "have the correct new title" do
          expected_title = FactoryGirl.build(:DDBIdentifier, :name => @new_name).titleize
    ...
    context "identifier mapping" do
      ...
      should "map the first identifier" do
        bgu_1_1 = FactoryGirl.build(:DDBIdentifier, :name => "papyri.info/ddbdp/bgu;1;1")
        assert_path_equal %w{bgu bgu.1 bgu.1.1.xml}, bgu_1_1.to_path
      end
      
      should "map ambiguous collections" do
        bgu_ppetr_2_1 = FactoryGirl.build(:DDBIdentifier, :name => "papyri.info/ddbdp/p.petr;2;1")
        assert_path_equal %w{p.petr p.petr.2 p.petr.2.1.xml}, bgu_ppetr_2_1.to_path
        
        bgu_ppetr2_1 = FactoryGirl.build(:DDBIdentifier, :name => "papyri.info/ddbdp/p.petr.2;;1")
      ...
      #   chla_5_299FrA_B_C = FactoryGirl.build(:DDBIdentifier, :name => "papyri.info/ddbdp/0279:5:299FrA+B+C")
      #   assert_path_equal %w{chla chla.5 chla.5.299FrA+B+C.xml}, chla_5_299FrA_B_C.to_path
      # end
      
      should "map files with ',' in the identifier" do
        bgu_13_2230_1 = FactoryGirl.build(:DDBIdentifier, :name => "papyri.info/ddbdp/bgu;13;2230,1")
        assert_path_equal %w{bgu bgu.13 bgu.13.2230-1.xml}, bgu_13_2230_1.to_path
      end
      
      should "map files with '/' in the identifier" do
        o_bodl_2_1964_1967 = FactoryGirl.build(:DDBIdentifier, :name => "papyri.info/ddbdp/o.bodl;2;1964/1967")
      ...
      #   no_series = FactoryGirl.build(:DDBIdentifier, :name => "papyri.info/ddbdp/;1;1")
      #   assert_raise RuntimeError do
      #     no_series.to_path
      #   end
      #   
      #   nonsense_series = FactoryGirl.build(:DDBIdentifier, :name => "papyri.info/ddbdp/3735928559:1:1")
      #   assert_raise RuntimeError do
      ...
      #         this_ddb = FactoryGirl.build(:DDBIdentifier, :name => identifier_name)
