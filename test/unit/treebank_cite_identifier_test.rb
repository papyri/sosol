...
class TreebankCiteIdentifierTest < ActiveSupport::TestCase
  ...
  context "identifier test" do
    setup do
      @creator = Factory(:user, :name => "Creator4")
    ...
    should "create from template in repo" do      
      ...
      assert_equal REXML::XPath.first(template_xml,"/treebank/annotator/uri").text.strip, "http://data.perseus.org/users/Creator4"
      assert_not_nil REXML::XPath.first(template_xml,"/treebank/date")
      

    end
   
    # TODO reenable this test when we have a solid external url to use
    # or wire up a mock
    #should "create from url" do      
    #  test = TreebankCiteIdentifier.new_from_template(@publication,"urn:cite:perseus:testcoll",["http://raw.githubusercontent.com/PerseusDL/treebank_data/master/v1.6/greek/data/tlg0008.tlg001.perseus-grc1.tb.xml"])
    #  assert_not_nil test.xml_content
