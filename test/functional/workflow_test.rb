...
class WorkflowTest < ActiveSupport::TestCase
    ...
    context "a publication" do
      ...
      context "submitted with only DDB modifications" do
        setup do
          ...
          @publication.reload
          @publication.submit
          Rails.logger.info "______ PUBLICATION _____"
  ...
  context "for Community Refactor" do
    setup do
      @community = FactoryGirl.create(:master_community, :is_default => true )
      @ddb_board = FactoryGirl.create(:community_board, :title => 'DDbDP Editorial Board', :community => @community)
