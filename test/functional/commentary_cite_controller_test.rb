...
class CommentaryCiteIdentifiersControllerTest < ActionController::TestCase
  def setup
    ...
    @publication = Factory(:publication, :owner => @user, :creator => @user, :status => "new")
  ...
  def test_should_fail_duplicate_target
    get :create, :publication_id => @publication.id, :urn => "urn:cite:perseus:testcoll", :init_value => "http://data.perseus.org/citations/urn:cts:latinLit:phi0959.phi006:1.253-1.415"
