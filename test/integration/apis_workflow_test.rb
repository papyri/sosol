  ...
  context 'for idp3' do
    context 'apis testing' do
      setup do
        ...
        @apis_board.rank = 4
        [@meta_board, @text_board, @translation_board, @apis_board].each(&:save)
      end
      ...
      should 'user creates and submits publication to sosol' do
        ...
        assert_equal 1, apis_publications.length,
                     "APIS does not have 1 publication but rather, #{apis_publications.length} publications"
        ...
        apis_publication.identifiers.each do |id|
          apis_identifier = id if @apis_board.controls_identifier?(id)
        end
        ...
        open_session do |apis_session|
          apis_session.post "/publications/vote/#{apis_publication.id}", params: {
        ...
        apis_final_publication.identifiers.each do |id|
          apis_final_identifier = id if @apis_board.controls_identifier?(id)
        end
